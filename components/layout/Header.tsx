      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3.5 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-3" aria-label={`${business.name} home`}>
          ...
          <span className="min-w-0">
            <span className="block truncate text-sm font-black leading-4 text-slate-950 sm:text-base">{business.name}</span>
            <span className="block truncate text-xs font-semibold text-slate-500">Medical equipment Mumbai</span>
          </span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2 lg:hidden">
          <Link href="/cart" className="inline-flex h-10 items-center justify-center rounded-lg bg-[#047068]/10 px-3 text-sm font-black text-[#047068]">
            Cart ({count})
          </Link>
          <Button className="h-10 px-3" onClick={() => setOpen((value) => !value)} variant="secondary" aria-label="Toggle menu">
            Menu
          </Button>
        </div>
      </div>
      {open ? (
        <div className="max-h-[calc(100vh-76px)] overflow-y-auto border-t border-slate-200 bg-white px-4 py-3 lg:hidden">
          <nav className="grid gap-1">
            ...
            {count > 0 ? (
              <Link href="/checkout" onClick={() => setOpen(false)} className="rounded-lg bg-[#047068] px-3 py-3 text-center font-black text-white">
                Purchase / Checkout
              </Link>
            ) : null}
            {userEmail ? (
