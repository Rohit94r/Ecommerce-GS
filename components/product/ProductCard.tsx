...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group overflow-hidden rounded-md border border-[#047068]/15 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-5">
        ...
        <Link href={`/products/${product.id}`}>
          <h3 className="mt-2 text-xl font-bold leading-snug text-slate-950 transition hover:text-[#047068]">{product.name}</h3>
        </Link>
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">{product.description}</p>
        <p className="mt-3 text-xs font-black uppercase tracking-[0.14em] text-slate-500">{product.brand} | Mumbai support</p>
